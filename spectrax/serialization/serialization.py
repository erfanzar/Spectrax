...
"""Low-level TensorStore serialization primitives.

Provides :func:`tree_serialize_leaves` and :func:`tree_deserialize_leaves`
which wrap JAX's :class:`GlobalAsyncCheckpointManager` for array I/O,
along with helper utilities for mapping PyTree leaves to filesystem paths
and reconstructing nested dictionaries from checkpoint indexes.
"""
...
def tree_serialize_leaves(
    ...
    *,
    ...
    if write_index and array_info and fsspec_utils.should_write_shared_checkpoint_files(checkpoint_dir):
        index_path = _fs.joinpath(checkpoint_dir, "tensorstore_index.json")
        if _fs.exists(index_path):
        ...
        _fs.write_text(index_path, json.dumps(index_data, indent=2))
    ...
    *,
    ...
    if _fs.exists(index_path):
        ...
        for info in array_info:
            rel_path = info["path"]
            abs_path = _fs.joinpath(checkpoint_dir, rel_path)
            ...
            if key in shardings:
                ...
                if (
                    ...
                    raise ValueError(
                        f"No sharding specified for '{key}' on multi-axis mesh. Provide 'shardings' or 'sharding_rules'."
                    )
            ...
            for k in keys:
                ...
                for pattern, sharding in sharding_rules:
                    if _re.search(pattern, k.replace(".", "/")):
                        found = sharding
                        break
                matched_shardings.append(found if found is not None else NamedSharding(mesh=mesh, spec=PartitionSpec()))
            apply_tree_shardings = matched_shardings
        result = {}
        if paths_to_load:
